...
function _M.translate_chain(chain)

end

function _M.translate_chains(chains)
	...
	for i = 1, #chains do
		...
		local translation = _M.translate_chain(chain)

		local phase = _M.figure_phase(translation)

		table.insert(lua_resty_waf_chains[phase], translation)
	end
